...
const char* uniLinksPrefix = "irisremote://";
...
{
  HINSTANCE hInstance = LoadLibraryA("libirisremote.dll");
  if (!hInstance)
  {
    std::cout << "Failed to load libirisremote.dll." << std::endl;
    return EXIT_FAILURE;
  }
  FUNC_RUSTDESK_CORE_MAIN irisremote_core_main =
      (FUNC_RUSTDESK_CORE_MAIN)GetProcAddress(hInstance, "irisremote_core_main_args");
  if (!irisremote_core_main)
  {
    std::cout << "Failed to get irisremote_core_main." << std::endl;
    return EXIT_FAILURE;
  }
  ...
  int args_len = 0;
  char** c_args = irisremote_core_main(&args_len);
  if (!c_args)
  {
    std::string args_str = "";
  ...
  // Uri links dispatch
  HWND hwnd = ::FindWindow(_T("FLUTTER_RUNNER_WIN32_WINDOW"), _T("IrisRemote"));
  if (hwnd != NULL) {
  ...
  if (!window.CreateAndShow(
          is_cm_page ? L"IrisRemote - Connection Manager" : L"IrisRemote", origin,
          size, !is_cm_page)) {
      return EXIT_FAILURE;
  }

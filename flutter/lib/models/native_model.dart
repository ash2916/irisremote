...
class PlatformFFI {
  ...
  Future<void> init(String appType) async {
    ...
    final dylib = Platform.isAndroid
        ? DynamicLibrary.open('libirisremote.so')
        : Platform.isLinux
            ? DynamicLibrary.open('libirisremote.so')
            : Platform.isWindows
                ? DynamicLibrary.open('libirisremote.dll')
                : Platform.isMacOS
                    ? DynamicLibrary.open("liblibirisremote.dylib")
  ...
  void _startListenEvent(RustdeskImpl irisremoteImpl) {
    final appType = _appType == kAppTypeDesktopRemote ? '$_appType,$kWindowId' : _appType;
    var sink = irisremoteImpl.startGlobalEventStream(appType: appType);
    sink.listen((message) {
      () async {
        try {

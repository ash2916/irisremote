...
#[inline]
fn stop_irisremote_servers() {
    let _ = run_cmds(&format!(
        r##"ps -ef | grep -E 'irisremote +--server' | awk '{{printf("kill -9 %d\n", $2)}}' | bash"##,
    ));
}
...
fn stop_subprocess() {
    let _ = run_cmds(&format!(
        r##"ps -ef | grep '/etc/irisremote/xorg.conf' | grep -v grep | awk '{{printf("kill -9 %d\n", $2)}}' | bash"##,
    ));
    let _ = run_cmds(&format!(
        r##"ps -ef | grep -E 'irisremote +--cm-no-ui' | grep -v grep | awk '{{printf("kill -9 %d\n", $2)}}' | bash"##,
    ));
}
...
// stop_irisremote_servers() is just a temp solution here.
fn force_stop_server() {
    stop_irisremote_servers();
...
pub fn start_os_service() {
    ...
    while running.load(Ordering::SeqCst) {
        ...
        if keeps_headless || keeps_session {
            // for fixing https://github.com/irisremote/irisremote/issues/3129 to avoid too much dbus calling,
            sleep_millis(500);
        } else {
            sleep_millis(super::SERVICE_INTERVAL);
        }
...
pub fn check_super_user_permission() -> ResultType<bool> {
    let file = "/usr/share/irisremote/files/polkit";
    let arg;
    ...
    // https://github.com/irisremote/irisremote/issues/2756
    if let Ok(status) = Command::new("pkexec").arg(arg).status() {
        // https://github.com/irisremote/irisremote/issues/5205#issuecomment-1658059657s
...
mod desktop {
    ...
    pub struct Desktop {
        ...
        pub display: String,
        pub xauth: String,
        pub is_irisremote_subprocess: bool,
    }
    ...
    impl Desktop {
        ...
        pub fn is_headless(&self) -> bool {
            self.sid.is_empty() || self.is_irisremote_subprocess
        }
        ...
        fn set_is_subprocess(&mut self) {
            self.is_irisremote_subprocess = false;
            let cmd = "ps -ef | grep 'irisremote/xorg.conf' | grep -v grep | wc -l";
            if let Ok(res) = run_cmds(cmd) {
                if res.trim() != "0" {
                    self.is_irisremote_subprocess = true;
                }
            }
        }
...
fn switch_service(stop: bool) -> String {
    ...
    if home != "/root" && !Config::get().is_empty() {
        format!("cp -f {home}/.config/irisremote/IrisRemote.toml /root/.config/irisremote/; cp -f {home}/.config/irisremote/IrisRemote2.toml /root/.config/irisremote/;")
    } else {
        "".to_owned()
    }
}
...
pub fn uninstall_service(show_new_window: bool) -> bool {
    ...
    if !run_cmds_pkexec(&format!(
        "systemctl disable irisremote; systemctl stop irisremote; {cp}"
    )) {
...
pub fn install_service() -> bool {
    ...
    if !run_cmds_pkexec(&format!(
        "{cp} systemctl enable irisremote; systemctl start irisremote;"
    )) {
...
fn check_if_stop_service() {
    if Config::get_option("stop-service".into()) == "Y" {
        allow_err!(run_cmds(
            "systemctl disable irisremote; systemctl stop irisremote"
        ));
    }
}
...
pub fn check_autostart_config() -> ResultType<()> {
    ...
    let file = format!("{path}/irisremote.desktop");
